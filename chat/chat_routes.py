...
async def agent_chat(
    ...
    # #V2.0: 自动截取约 30% 的历史对话作为上下文
    # histories = ""
    # context_ratio = 0.6  # 使用 60% 的历史对话
    # recent_history = []

    # if parsed_history:
    #     total_messages = len(parsed_history)
    #     # 计算需要保留的消息数量（向上取整，确保至少有 1 条）
    #     messages_to_keep = max(2, int(total_messages * context_ratio))
        
    #     # 确保是偶数条（保持对话完整性：用户+AI）
    #     if messages_to_keep % 2 != 0:
    #         messages_to_keep -= 1
        
    #     # 如果计算后为 0，至少保留最新的 2 条（一轮对话）
    #     if messages_to_keep == 0 and total_messages >= 2:
    #         messages_to_keep = 2
    #     # 取最新的消息
    #     start_index = max(0, total_messages - messages_to_keep)
    #     recent_history = parsed_history[start_index:]
    #     print(f"📊 历史对话管理：总共 {total_messages} 条，保留 {messages_to_keep} 条 ({context_ratio*100:.0f}%)")
    
    # # 估算 token 使用情况
    # history_tokens = len(histories) // 2 if histories else 0
    # doc_tokens = len(documents) // 2 if documents else 0
    # estimated_context_tokens = history_tokens + doc_tokens
    
    # print(f"📊 Token 估算 - 历史：~{history_tokens}, 文档：~{doc_tokens}, 总计：~{estimated_context_tokens}")
    #     # 格式化历史对话
    ...
    #V1.0: 将历史对话转换为字典
    # history = [literal_eval(item) for item in history]  #将字符串转换为对应的 Python 对象(数字字符等)
    # # 控制历史记录长度，保留指定数量的消息
    # histories = ""
    # if history_len > 0:
    #     history = history[-2 * history_len:]    #取最后的2的倍数条历史对话

    # #在消息列表中加入历史消息，格式化历史对话
    # for msg in history:
    #     role = msg.get("role")
    #     content = msg.get("content")
    #     histories += f"{role}:{content}\n\n"
    ...
    async def agent_chat_iterator() -> AsyncIterable[str]:
        ...
        agent = create_react_agent(chat_model, tools, chat_prompt)
